...
## Full-dimension normalization correction

The retained MSC01 profile comparison revealed an additional reduction-order
mismatch at the production seed count, D=1,175. Binary correlation profiles
already matched exactly. The previous normalized arrays differed from the
independently executed source in 87,178,897 and 87,109,369 elements for the two
runs, with maximum absolute errors 5.662441e-6 and 6.079674e-6. These comparisons
and their source snapshot remain preserved as the original benchmark evidence.

A normalization-only experiment found that Fortran-order float32 reductions
before boolean row indexing match every upstream value in both complete runs.
Explicit sequential feature accumulation gives the same result. Boolean
selection `series[active]` had produced a C-contiguous temporary, causing NumPy's
fast-axis pairwise norm reduction to differ from the source's feature-order sum.
The correction forces Fortran layout, computes the mean and squared norm before
row selection, and uses explicit float32 sequential accumulation for the
one-row case, where Fortran and C contiguity coincide. The CBIG centered-zero
predicate remains unchanged. No EM equation or tolerance changed.

A new independent oracle, generated by
`validation/generate_normalization_high_dim.py`, executes the unchanged pinned
normalization body in GNU Octave 6.4.0. It covers 16 profiles at D=1,175, varied
binary sparsity, a continuous row that exercises mean accumulation, zero input,
a masked cortical row, a nonconstant row with a centered zero, and separately
executed one-row calls. New files have `normalization_high_dim` names; existing
synthetic fixtures, generators, and hashed adapters were not modified.

The new regression first produced five failures in the inherited calculation.
After correction, all nine new tests pass with exact array equality for C- and
F-order inputs and one-row calls. The combined existing Buckner profile,
Buckner pipeline, synthetic EM oracle and profile oracle tests, plus the new
fixture tests, pass **89 tests** (0.62 s). The new provenance JSON records source,
adapter, generator, input/output, fixture and runtime-log checksums. This
normalization improvement does not by itself establish that the original
single-subject EM comparison now converges identically; that requires a fresh
benchmark with the corrected source hash.
